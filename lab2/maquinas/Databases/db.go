...
func Peers(id string) []string {
	peers := []string{}
	if id != "DB1" {
		peers = append(peers, "10.35.168.44:50061")
	}
	if id != "DB2" {
		peers = append(peers, "10.35.168.45:50062")
	}
	if id != "DB3" {
		peers = append(peers, "10.35.168.46:50063")
	}
	return peers
}
...
func RegistrarConBroker(client pb.RegistroEntidadesClient, server *DBNodeServer) {
	...
	if publicIP == "" {
		fmt.Println("ERROR: Debes setear PUBLIC_IP en el contenedor de la DB")
		os.Exit(1)
	}
	addressToRegister := publicIP + *dbNodePort
