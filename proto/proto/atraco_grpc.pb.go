...
const (
	TerceraFase_IniciarGolpe_FullMethodName = "/TerceraFase/IniciarGolpe"
)

// TerceraFaseClient is the client API for TerceraFase service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
//
// ====== FASE 3 ======
type TerceraFaseClient interface {
	// Michael ordena iniciar el golpe; el personaje confirma recepción/inicio.
	IniciarGolpe(ctx context.Context, in *OrdenGolpe, opts ...grpc.CallOption) (*AckInicio, error)
}

type terceraFaseClient struct {
	cc grpc.ClientConnInterface
}

func NewTerceraFaseClient(cc grpc.ClientConnInterface) TerceraFaseClient {
	return &terceraFaseClient{cc}
}

func (c *terceraFaseClient) IniciarGolpe(ctx context.Context, in *OrdenGolpe, opts ...grpc.CallOption) (*AckInicio, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(AckInicio)
	err := c.cc.Invoke(ctx, TerceraFase_IniciarGolpe_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// TerceraFaseServer is the server API for TerceraFase service.
// All implementations must embed UnimplementedTerceraFaseServer
// for forward compatibility.
//
// ====== FASE 3 ======
type TerceraFaseServer interface {
	// Michael ordena iniciar el golpe; el personaje confirma recepción/inicio.
	IniciarGolpe(context.Context, *OrdenGolpe) (*AckInicio, error)
	mustEmbedUnimplementedTerceraFaseServer()
}

// UnimplementedTerceraFaseServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedTerceraFaseServer struct{}

func (UnimplementedTerceraFaseServer) IniciarGolpe(context.Context, *OrdenGolpe) (*AckInicio, error) {
	return nil, status.Errorf(codes.Unimplemented, "method IniciarGolpe not implemented")
}
func (UnimplementedTerceraFaseServer) mustEmbedUnimplementedTerceraFaseServer() {}
func (UnimplementedTerceraFaseServer) testEmbeddedByValue()                     {}

// UnsafeTerceraFaseServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to TerceraFaseServer will
// result in compilation errors.
type UnsafeTerceraFaseServer interface {
	mustEmbedUnimplementedTerceraFaseServer()
}

func RegisterTerceraFaseServer(s grpc.ServiceRegistrar, srv TerceraFaseServer) {
	// If the following call pancis, it indicates UnimplementedTerceraFaseServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&TerceraFase_ServiceDesc, srv)
}

func _TerceraFase_IniciarGolpe_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(OrdenGolpe)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(TerceraFaseServer).IniciarGolpe(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: TerceraFase_IniciarGolpe_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(TerceraFaseServer).IniciarGolpe(ctx, req.(*OrdenGolpe))
	}
	return interceptor(ctx, in, info, handler)
}

// TerceraFase_ServiceDesc is the grpc.ServiceDesc for TerceraFase service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var TerceraFase_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "TerceraFase",
	HandlerType: (*TerceraFaseServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "IniciarGolpe",
			Handler:    _TerceraFase_IniciarGolpe_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "atraco.proto",
